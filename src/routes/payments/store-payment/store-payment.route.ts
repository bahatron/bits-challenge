...
import { CheckAuthMiddleware } from "../../../server/middleware/auth-checker";

export const StorePaymentRoute: RequestHandler[] = [
    CheckAuthMiddleware(["write"]),
    async (req, res) => {
        Logger.debug({
            data: req.body,
        ...
        let data = {
            ...
            payment_currency: {
                [AttributeType.STRING]: req.body.payment_currency,
            },
            payment_amount: {
        ...
        return res.status(201).json(req.body.payment_id);
    },
];
